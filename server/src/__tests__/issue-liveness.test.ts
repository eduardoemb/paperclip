...
describe("issue graph liveness classifier", () => {
  ...
  it("ignores cancelled blockers and detects uninvokable blocker assignees deterministically", () => {
  ...
  it("does not flag a cancelled blocker on an assigned todo source", () => {
    const findings = classifyIssueGraphLiveness({
      issues: [
        issue({ status: "todo" }),
        issue({
  ...
  it("ignores a cancelled blocker and evaluates the in-review source action path", () => {
    const findings = classifyIssueGraphLiveness({
      issues: [
